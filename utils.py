...
def clustering(feature, cluster_num, device):
    ...
    silhouette_avg = silhouette_score(feature.cpu().numpy(), predict_labels.cpu().numpy())
    davies_bouldin_avg = davies_bouldin_score(feature.cpu().numpy(), predict_labels.cpu().numpy())
    calinski_harabasz_avg = calinski_harabasz_score(feature.cpu().numpy(), predict_labels.cpu().numpy())

    # nmi, ari = eva(true_labels, predict_labels.numpy(), show_details=False)
    return silhouette_avg,davies_bouldin_avg,calinski_harabasz_avg,predict_labels.numpy(), centers, dis
